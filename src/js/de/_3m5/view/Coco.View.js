...
module.exports = Coco.View = dejavu.Class.declare({
  ...
  initialize: function ($model, $syncModelWithForms) {
    ...
    if (this._getRouter() != null) {
      this._getRouter().addEventListener(Coco.RouterEvent.HIDE_VIEW + this.$name, () => {
        this.showLoading();
      });
    }
  ...
  render: function () {
    ...
    if (coco.config && coco.config.locale) {
      localeObject = {
        data: {
          intl: {
            locales: coco.config.locale
          }
        }
      };
    }
  ...
  _getHBSModel: function () {
    ...
    if (this._collection != null || this._model != null) {
      ...
      props = (this._collection == null) ? this._model.getAttributes() : {
        collection: {
          models: this._collection.getAllAttributes(),
          size: this._collection.size()
        }
      };
    }
